...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main>
      {view === "units" && <section className="units-view"><p className="eyebrow">Study by repetition</p><h1>Choose a unit.</h1><div className="unit-grid">{bank.map((entry, index) => { const isLocked = index > 1; const lockedStyle = isLocked ? { background: "#d8d9d6", borderColor: "#c3c5c0", color: "#747873", cursor: "not-allowed", filter: "grayscale(1)" } : undefined; return <button className={`unit-card ${isLocked ? "locked" : ""}`} onClick={() => openUnit(index)} disabled={isLocked} style={lockedStyle} key={entry.unit}><span className="unit-number" style={isLocked ? { color: "#747873" } : undefined}>UNIT {index + 1}</span><span className="unit-title">{entry.unit}</span><span className="question-count" style={isLocked ? { color: "#747873" } : undefined}>{isLocked ? "Locked" : `${entry.questions.length} questions`}</span></button>; })}</div></section>}
